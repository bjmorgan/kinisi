...
displacements = sc.Variable(values=np.array([1, 1.5, 2, 2.5, 3, 3.5, 4, 4.5]).reshape((4, 2, 1)),
                            dims=['obs', 'atom', 'dimension'])
...
class TestCalculateMsd(unittest.TestCase):
    ...
    def test_calculate_msd(self):
        ...
        expected = sc.DataArray(data=sc.Variable(dims=['time interval'],
                                ...
                                coords={
                                    ...
                                    'n_samples':
                                    sc.array(values=[8.0, 4.0, 8 / 3, 2], dims=['time interval'], unit='dimensionless')
                                })
...
class TestCalculateMstd(unittest.TestCase):
    ...
    def test_calculate_mstd(self):
        ...
        expected = sc.DataArray(data=sc.Variable(dims=['time interval'],
                                ...
                                coords={
                                    'time interval':
                                    PARSER.dt['time interval', :-1],
                                    'dimensionality':
                                    PARSER.dimensionality,
                                    'n_samples':
                                    sc.array(values=[8 / 2, 4 / 2, 8 / 6], dims=['time interval'], unit='dimensionless')
                                })
        testing.assert_allclose(result, expected)
