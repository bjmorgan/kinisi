...
dc = np.random.random(size=(100, 50, 3))
indices = np.arange(0, 50, 1, dtype=int)
time_step = 1.0
step_skip = 1
...
class TestParser(unittest.TestCase):
    ...
    def test_pymatgen_init(self):
        ...
        assert_equal(data.indices.values, np.arange(xd.natoms[0]))

    def test_pymatgen_init_with_indices(self):
        xd = Xdatcar(os.path.join(os.path.dirname(kinisi.__file__), 'tests/inputs/example_XDATCAR.gz'))
        da_params = {'specie': None, 'time_step': 2.0, 'step_skip': 50, 'specie_indices': sc.Variable(dims=['indices'],values=[3, 4, 5])}
        data = PymatgenParser(xd.structures, **da_params)
        assert_almost_equal(data.time_step, 2.0)
        assert_almost_equal(data.step_skip, 50)
        assert_equal(data.indices.values, [3, 4, 5])

    def test_pymatgen_init_with_molecules(self):
        xd = Xdatcar(os.path.join(os.path.dirname(kinisi.__file__), 'tests/inputs/example_XDATCAR.gz'))
        molecules = sc.Variable(dims=['group_of_atoms','atom'], values=[[2, 3, 4], [5, 6, 7]])
        da_params = {'specie': None, 'time_step': 2.0, 'step_skip': 50, 'specie_indices': molecules}
        data = PymatgenParser(xd.structures, **da_params)
        assert_almost_equal(data.time_step, 2.0)
        assert_almost_equal(data.step_skip, 50)
        assert_equal(data.indices.values, list(range(len(molecules))))
    ...
    def test_pymatgen_big_time_interval(self):
        ...
        assert_equal(data.indices.values, list(range(xd.natoms[0])))
    ...
    #    da_params = {'specie': None, 'time_step': 1.0, 'step_skip': 1, 'specie_indices': sc.Variable(dims=['group_of_atoms','atom'], values=[[1, 2, 3]])}
    #    data = PymatgenParser(xd.structures, **da_params)
    ...
    # #     data_1 = PymatgenParser(xd.structures, **da_1_params)
    ...
    # #     data_1 = ASEParser(traj, **da_1_params)
    # #     assert_almost_equal(data_1.time_step, 1)
    # #     assert_almost_equal(data_1.step_skip, 1)
    # #     assert_equal(data_1.indices, [0])
    # #     assert_equal(data_1.drift_indices, [])
    ...
    # #     data_2 = ASEParser(traj, **da_2_params)
    # #     assert_almost_equal(data_2.time_step, 1)
    # #     assert_almost_equal(data_2.step_skip, 1)
    # #     assert_equal(data_2.indices, [0])
    # #     assert_equal(data_2.drift_indices, list(range(1, 9)))
