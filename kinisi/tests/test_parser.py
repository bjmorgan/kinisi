...
dcoords = np.random.random(size=(100, 100, 3))
ddisp = np.cumsum(dcoords, axis=1)
...
class TestParser(unittest.TestCase):
    ...
    def test_parser_init_timestep(self):
        p = parser.Parser(dcoords, ddisp, indices, [], time_step, step_skip, get_msd=False, min_dt=20)
    ...
    def test_parser_init_coords(self):
        p = parser.Parser(dcoords, ddisp, indices, [], time_step, step_skip, get_msd=False, min_dt=20)
        assert_equal(p.coords, dcoords)
    
    def test_parser_init_min_dt(self):
        p = parser.Parser(dcoords, ddisp, indices, [], time_step, step_skip, min_dt=20)
        assert_equal(p.min_dt, 20)
    
    def test_parser_delta_t(self):
        p = parser.Parser(dcoords, ddisp, indices, [], time_step, step_skip, get_msd=True, min_dt=20)
    ...
    def test_smoothed_timesteps_not_enough(self):
        with self.assertRaises(ValueError):
            p = parser.Parser(dcoords, ddisp, indices, [], time_step, step_skip, min_dt=120)
            p.smoothed_timesteps(100, 20, indices)

    def test_smoothed_timesteps_min_dt_zero(self):
        p = parser.Parser(dcoords, ddisp, indices, [], time_step, step_skip, min_dt=0)
        timesteps = p.smoothed_timesteps(100, 30, indices)
    ...
    def test_correct_drift_no_framework(self):
        p = parser.Parser(dcoords, ddisp, indices, [], time_step, step_skip, min_dt=20)
        corrected = p.correct_drift([], ddisp)
    ...
    def test_correct_drift_framework(self):
        p = parser.Parser(dcoords, ddisp, indices, [0, 1, 2], time_step, step_skip, min_dt=20)
        corrected = p.correct_drift([], ddisp)
        assert_equal(len(corrected), 100)
        for i, d in enumerate(corrected):
            assert_equal(d.shape[0], 100)
    ...
    def test_get_disps(self):
        p = parser.Parser(dcoords, ddisp, indices, [], time_step, step_skip, min_dt=20)
        dt, disp_3d = p.get_disps(np.arange(20, 100, 1), ddisp)
